...
var SearchResultItemViewModel = function SearchResultItemViewModel(item) {
    ...
    Object(item).keys().forEach(function (key) {
        _this[key] = knockout.observable(item[key]);
    });
};

var SearchPageViewModel = function () {
    function SearchPageViewModel(_ref) {
        var keyword = _ref.keyword,
            search = _ref.actions.search;
        classCallCheck(this, SearchPageViewModel);

        this.keyword = knockout.observable(keyword);
    ...
    createClass(SearchPageViewModel, [{
        key: '_populateResults',
        value: function _populateResults(results) {
            var _this2 = this;

            results.forEach(function (item) {
                _this2.results.push(new SearchResultItemViewModel(item));
            });
        }
    }, {
        key: 'search',
        value: function search() {
            ...
            var search = this.actions.search;
            ...
            this._viewModel = new SearchPageViewModel({
                ...
                actions: {
                    ...
                    search: function search(_ref) {
                        ...
                        return _this2._client.searchIdeas({ keyword: keyword, page: page, pageSize: pageSize });
                    }
                }
            });

            knockout.applyBindings(this._viewModel, document.getElementById('search-container'));
